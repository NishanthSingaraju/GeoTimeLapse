  ...
  with row1_col2:
    ...
    if collection == "Any Earth Engine ImageCollection":
      ...
      if MODIS_option == "Daytime (1:30 pm)":
        st.session_state[
            "ee_asset_id"] = "projects/sat-io/open-datasets/gap-filled-lst/gf_day_1km"
      else:
        st.session_state[
            "ee_asset_id"] = "projects/sat-io/open-datasets/gap-filled-lst/gf_night_1km"
    ...
    add_outline = st.checkbox("Overlay an administrative boundary on timelapse",
                              False)
  ...
  with row1_col1:
    ...
    if sample_roi != "Uploaded GeoJSON":
      ...
      if collection in [
        ...
        gdf = gpd.GeoDataFrame(index=[0],
                               crs=crs,
                               geometry=[ocean_rois[sample_roi]])
    ...
    if collection in [
      ...
      with st.form("submit_goes_form"):
        ...
        if sample_roi == "Uploaded GeoJSON":
          ...
          roi_start_date = datetime.datetime.strptime(roi_start[:10],
                                                      "%Y-%m-%d")
          roi_end_date = datetime.datetime.strptime(roi_end[:10], "%Y-%m-%d")
          roi_start_time = datetime.time(int(roi_start[11:13]),
                                         int(roi_start[14:16]))
          roi_end_time = datetime.time(int(roi_end[11:13]), int(roi_end[14:16]))
        ...
        with st.expander("Customize timelapse"):
          ...
          scan_type = st.selectbox("Select a scan type:",
                                   ["Full Disk", "CONUS", "Mesoscale"])

          start_time = st.time_input("Select the start time of the start date:",
                                     roi_start_time)

          end_time = st.time_input("Select the end time of the end date:",
                                   roi_end_time)

          start = (start_date.strftime("%Y-%m-%d") + "T" +
                   start_time.strftime("%H:%M:%S"))
          end = (end_date.strftime("%Y-%m-%d") + "T" +
      ...
      with st.form("submit_modis_form"):
        ...
        with st.expander("Customize timelapse"):
          ...
          start = st.date_input("Select a start date:",
      ...
      with st.form("submit_ts_form"):
        with st.expander("Customize timelapse"):
          ...
          vis_params = st.text_area(
              ...
              "",
              help=
              "Enter a string in the format of a dictionary, such as '{'min': 23, 'max': 32}'",
          )
          ...
          colorbar_label = st.text_input("Enter the colorbar label:",
      ...
      with st.form("submit_naip_form"):
        with st.expander("Customize timelapse"):
          ...
          bands = st.selectbox("Select a band combination:", ["N/R/G", "R/G/B"],
                               index=0)
